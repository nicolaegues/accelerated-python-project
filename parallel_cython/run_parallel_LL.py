...
import LebwohlLasher_cy_parallel
import sys
import time
import datetime
import numpy as np
...
def plotdat(arr, pflag, nmax):
    ...
    if pflag==1: # colour the arrows according to energy
        ...
        for i in range(nmax):
            for j in range(nmax):
                cols[i,j] = LebwohlLasher_cy_parallel.one_energy(arr,i,j,nmax)
...
def main(program, nsteps, nmax, temp, pflag, threads):
    ...
    energy[0] = LebwohlLasher_cy_parallel.all_energy(lattice,nmax, threads)
    ratio[0] = 0.5 # ideal value
    order[0] = LebwohlLasher_cy_parallel.get_order(lattice,nmax, threads)

    # Begin doing and timing some MC steps.
    initial = time.time() 
    for it in range(1,nsteps+1):
        ratio[it] = LebwohlLasher_cy_parallel.MC_step(lattice,temp,nmax, threads)
        energy[it] = LebwohlLasher_cy_parallel.all_energy(lattice,nmax, threads)
        order[it] = LebwohlLasher_cy_parallel.get_order(lattice,nmax, threads)
